...
class TestMoveValues : AbstractTestGameWithSmallNumbers() {
    ...
    fun myTest() {
/*
        val input = TestBoard("-2-- ---- -2-- -2--")
        val expected = TestBoard("-4-- -2-- ---- ----")
        testMove(UP, input, expected, true)

        val input1 = TestBoard("-2-- ---- -2-- -2--")
        val expected1 = TestBoard("---- ---- -2-- -4--")
        testMove(DOWN, input1, expected1, true)
*/
        val input2 = TestBoard("-224 ---- ---- ----")
        val expected2 = TestBoard("--44 ---- ---- ----")
        testMove(RIGHT, input2, expected2, true)
    }
